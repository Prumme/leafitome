...
export function Navbar() {
  ...
  async function handleLogout() {
    await logout()
    useTodoStore.setState({ todos: [], loaded: false })
    useHistoryStore.setState({ entries: [], loaded: false })
    useBadgeStore.setState({
      progress: { unlocked: {}, hasTraveled: false },
      loaded: false,
      pendingToasts: [],
    })
        ...
        <div className="mx-auto flex h-14 max-w-5xl items-center justify-between gap-3 px-4 sm:h-16 sm:px-6">
          <div className="flex items-center gap-2.5">
            ...
            <div className="leading-tight">
              ...
              <p className="hidden truncate text-xs text-ink-muted sm:block">
                {user?.displayName || user?.email || SITE.tagline}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-lg px-2.5 py-2 text-sm font-medium text-forest-700 hover:bg-forest-100"
